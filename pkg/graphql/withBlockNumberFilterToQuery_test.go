...
func TestWithBlockNumberFilter(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			startingBlock: 1000,
			endingBlock:   9999999,
			expectedQuery: constants.CollectionLog + " (filter: { _and: [ { blockNumber: { _ge: 1000 } }, { blockNumber: { _le: 9999999 } } ] }){ address topics data transactionHash }",
			expectError:   false,
		},
		{
			...
			startingBlock: 2000,
			endingBlock:   5000,
			expectedQuery: constants.CollectionLog + "(filter: { address: { _eq: \"0x123\" } }, _and: [ { blockNumber: { _ge: 2000 } }, { blockNumber: { _le: 5000 } } ]) { address topics data }",
			expectError:   false,
		},
		{
			...
			startingBlock: 3000,
			endingBlock:   6000,
			expectedQuery: constants.CollectionLog + "(filter: { }, _and: [ { blockNumber: { _ge: 3000 } }, { blockNumber: { _le: 6000 } } ]) { address topics data }",
			expectError:   false,
		},
		{
			...
			startingBlock: 1500,
			endingBlock:   8000,
			expectedQuery: constants.CollectionTransaction + " (filter: { _and: [ { blockNumber: { _ge: 1500 } }, { blockNumber: { _le: 8000 } } ] }){ hash from to value }",
			expectError:   false,
		},
		{
			...
			startingBlock: 5000,
			endingBlock:   10000,
			expectedQuery: constants.CollectionBlock + " (filter: { _and: [ { number: { _ge: 5000 } }, { number: { _le: 10000 } } ] }){ hash number timestamp }",
			expectError:   false,
		},
		{
			...
			startingBlock: 1000,
			endingBlock:   2000,
			expectedQuery: constants.CollectionAccessListEntry + " (filter: { _and: [ { transaction: { blockNumber: { _ge: 1000 } } }, { transaction: { blockNumber: { _le: 2000 } } } ] }){ address storageKeys }",
			expectError:   false,
		},
		{
			...
			startingBlock: 100,
			endingBlock:   500,
			expectedQuery: constants.CollectionLog + "  (filter: { _and: [ { blockNumber: { _ge: 100 } }, { blockNumber: { _le: 500 } } ] }){  address  topics  }",
			expectError:   false,
		},
		{
			name:        "empty query",
...
func TestWithBlockNumberFilter_ComplexFilters(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			startingBlock: 4000,
			endingBlock:   7000,
			expectedQuery: constants.CollectionLog + "(filter: { address: { _eq: \"0x123\" }, topics: { _in: [\"0xabc\"] } }, _and: [ { blockNumber: { _ge: 4000 } }, { blockNumber: { _le: 7000 } } ]) { address topics data }",
		},
		{
			...
			startingBlock: 5000,
			endingBlock:   9000,
			expectedQuery: constants.CollectionLog + "(filter: { address: { _eq: \"0x123\" }, _and: [{ topics: { _in: [\"0xabc\"] } }] }, _and: [ { blockNumber: { _ge: 5000 } }, { blockNumber: { _le: 9000 } } ]) { address topics data }",
		},
		{
			...
			startingBlock: 1000,
			endingBlock:   3000,
			expectedQuery: constants.CollectionAccessListEntry + "(filter: { address: { _eq: \"0x456\" } }, _and: [ { transaction: { blockNumber: { _ge: 1000 } } }, { transaction: { blockNumber: { _le: 3000 } } } ]) { address storageKeys }",
		},
		{
			...
			startingBlock: 2000,
			endingBlock:   4000,
			expectedQuery: constants.CollectionBlock + "(filter: { hash: { _eq: \"0x789\" } }, _and: [ { number: { _ge: 2000 } }, { number: { _le: 4000 } } ]) { hash number timestamp }",
		},
	}
...
func TestWithBlockNumberFilter_EdgeCases(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			startingBlock: 1000,
			endingBlock:   1000,
			expectedQuery: constants.CollectionLog + " (filter: { _and: [ { blockNumber: { _ge: 1000 } }, { blockNumber: { _le: 1000 } } ] }){ address topics }",
			expectError:   false,
		},
		{
			...
			startingBlock: 5000,
			endingBlock:   1000,
			expectedQuery: constants.CollectionLog + " (filter: { _and: [ { blockNumber: { _ge: 5000 } }, { blockNumber: { _le: 1000 } } ] }){ address topics }",
			expectError:   false,
		},
		{
			...
			startingBlock: 0,
			endingBlock:   1000,
			expectedQuery: constants.CollectionLog + " (filter: { _and: [ { blockNumber: { _ge: 0 } }, { blockNumber: { _le: 1000 } } ] }){ address topics }",
			expectError:   false,
		},
		{
			...
			expectedQuery: constants.CollectionLog + " (filter: { _and: [ { blockNumber: { _ge: 18446744073709551615 } }, { blockNumber: { _le: 18446744073709551615 } } ] }){ address topics }",
			expectError:   false,
		},
		{
			...
			startingBlock: 1000,
			endingBlock:   2000,
			expectedQuery: constants.CollectionTransaction + "(filter: { _and: [{ from: { _eq: \"0x123\" } }, { to: { _eq: \"0x456\" } }] }, _and: [ { blockNumber: { _ge: 1000 } }, { blockNumber: { _le: 2000 } } ]) { hash from to }",
			expectError:   false,
		},
	}
...
// ---------------------------------------------------------------------------
// WithBlockNumberFilter – unclosed parenthesis and no-brace-in-body paths
// ---------------------------------------------------------------------------

func TestWithBlockNumberFilter_UnclosedParenthesis(t *testing.T) {
	// Query with opening parenthesis but no matching close => error
	query := constants.CollectionLog + "(filter: { address: { _eq: \"0x123\" }"
	_, err := WithBlockNumberFilter(query, 1, 100)
	require.Error(t, err)
	require.Contains(t, err.Error(), "unable to find matching closing parenthesis")
}

func TestWithBlockNumberFilter_ExistingFilterEmpty_Block(t *testing.T) {
	// Block collection with existing empty filter args
	query := constants.CollectionBlock + "() { hash number }"
	result, err := WithBlockNumberFilter(query, 10, 20)
	require.NoError(t, err)
	require.Contains(t, result, "number: { _ge: 10 }")
	require.Contains(t, result, "number: { _le: 20 }")
}

func TestWithBlockNumberFilter_ExistingFilterEmpty_AccessListEntry(t *testing.T) {
...
func TestWithBlockNumberFilter_WhitespaceOnlyQuery(t *testing.T) {
	// Whitespace-only query is trimmed but still doesn't match the regex
	_, err := WithBlockNumberFilter("   ", 1, 100)
	require.Error(t, err)
	require.Contains(t, err.Error(), "unable to parse collection name")
}

func TestWithBlockNumberFilter_ExistingFilterEmpty_Generic(t *testing.T) {
...
func TestWithBlockNumberFilter_ExistingFilterEmpty_Transaction(t *testing.T) {
	// Transaction collection with existing empty filter args
	query := constants.CollectionTransaction + "() { hash from to }"
	result, err := WithBlockNumberFilter(query, 5, 15)
	require.NoError(t, err)
