...
func TestAddBlockNumberFilter(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			startingBlock: 1000,
			expectedQuery: "Log (filter: { blockNumber: { _gte: 1000 } }){ address topics data transactionHash }",
			expectError:   false,
		},
		{
			...
			startingBlock: 2000,
			expectedQuery: "Log(filter: { address: { _eq: \"0x123\" } }, blockNumber: { _gte: 2000 }) { address topics data }",
			expectError:   false,
		},
		{
			...
			startingBlock: 3000,
			expectedQuery: "Log(filter: { }, blockNumber: { _gte: 3000 }) { address topics data }",
			expectError:   false,
		},
		{
			...
			startingBlock: 1500,
			expectedQuery: "Transaction (filter: { blockNumber: { _gte: 1500 } }){ hash from to value }",
			expectError:   false,
		},
		{
			...
			startingBlock: 5000,
			expectedQuery: "Block (filter: { blockNumber: { _gte: 5000 } }){ hash number timestamp }",
			expectError:   false,
		},
		{
			...
			startingBlock: 100,
			expectedQuery: "Log  (filter: { blockNumber: { _gte: 100 } }){  address  topics  }",
			expectError:   false,
		},
		{
			name:        "empty query",
...
func TestAddBlockNumberFilter_ComplexFilters(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			startingBlock: 4000,
			expectedQuery: "Log(filter: { address: { _eq: \"0x123\" }, topics: { _in: [\"0xabc\"] } }, blockNumber: { _gte: 4000 }) { address topics data }",
		},
		{
			name:          "nested filter structure",
			query:         "Log(filter: { address: { _eq: \"0x123\" }, _and: [{ topics: { _in: [\"0xabc\"] } }] }) { address topics data }",
			startingBlock: 5000,
			expectedQuery: "Log(filter: { address: { _eq: \"0x123\" }, _and: [{ topics: { _in: [\"0xabc\"] } }] }, blockNumber: { _gte: 5000 }) { address topics data }",
		},
	}
