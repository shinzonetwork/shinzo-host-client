...
// three distinct paths on the same secp256k1 HD tree, one seed, unrelated-
// looking keys. coin type 60 (Ethereum) throughout, including for the
// paths that aren't really "Ethereum keys," since it's just a source of
// deterministic secp256k1 bytes at that point.
const (
...
func EnsureKeys(cfg *hostconfig.Config, recoverMnemonic string, log *zap.SugaredLogger) (NodeKeys, error) {
	...
	// mnemonic goes out of scope here. only the derived keys above were ever written to disk.
