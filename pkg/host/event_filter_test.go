...
// The whole point of this type: rules change live, no restart, no new
// filter object, same instance DefraDB already holds a reference to.
...
// Genuine concurrent access, not just sequential update-then-read. Run
// with -race, this is the actual claim being tested: safe to call
// AllowReplication from many goroutines while UpdateRules runs from
// another, the way DefraDB's own P2P worker pool would.
