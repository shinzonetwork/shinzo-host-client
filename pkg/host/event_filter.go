...
// NewEventFilter returns nil when disabled, same convention as the old
// NewEventReplicationFilter, so callers can hand the result straight to
// DefraDB, nil means "no filter."
...
// UpdateRules swaps the live rule set. Safe to call while the node is
// running and processing documents concurrently (atomic.Pointer, no
// lock needed), takes effect on the next document, no restart.
