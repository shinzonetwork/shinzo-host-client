...
docker run -d \
  ...
  -e DEFRADB_BLOCK_CACHE_MB=1024 \
  -e DEFRADB_MEMTABLE_MB=512 \
  -e DEFRADB_INDEX_CACHE_MB=512 \
  -e DEFRADB_NUM_COMPACTORS=4 \
  -e DEFRADB_NUM_LEVEL_ZERO_TABLES=30 \
  -e DEFRADB_NUM_LEVEL_ZERO_TABLES_STALL=60 \
  -e LOG_LEVEL=error \
  -e LOG_SOURCE=false \
